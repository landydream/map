...
var WorldSocketMgr = (function (_super) {
    ...
    WorldSocketMgr.prototype.connect = function (ip, port) {
        ...
        this.close(true);
        console.log("开始链接中央服webSocket ip=" + ip + " port=" + port);
        this._ip = ip;
        this._port = port;
        var httphead = document.location.protocol.indexOf("https");
        if (ip.indexOf("wss:") >= 0 || httphead != -1) {
            var url = "wss://" + ip + ":" + port + "/";
    ...
    WorldSocketMgr.prototype.onConnect = function (e) {
        ...
        //链接成功请求登录
        // GameGlobal.modelWorldNet.CG_Cross_loginCross_2633("123", Model_player.voMine.id);
    };
    ...
    WorldSocketMgr.prototype.doMsg = function () {
        ...
        while (this.socketCache.bytesAvailable >= 2) {
            ...
            if (this.socketCache.bytesAvailable >= len - 2) {
                ...
                var handler = GameGlobal.socketMgr.handleMap[cmd];
                if (!handler) {
                ...
                var requestID;
                var request = GameGlobal.socketMgr.request[requestID];
                if (request) {
                    try {
...
//# sourceMappingURL=WorldSocketMgr.js.map